...
##################################
ZIP=1992.83 # [m]
### our simulation "CAD assumption":
### Q0: zmid = (367.33336+(464.6664-367.33336)/2)/100 + 1992.83 = 1996.9899988
### Q1: zmid = (590.3336+(687.6664-590.3336)/2)/100   + 1992.83 = 1999.22
### Q2: zmid = (812.3336+(909.6664-812.3336)/2)/100   + 1992.83 = 2001.44
### from Doug's survey (2/10/25 and 2/3/25) [m]
q0z=1996.983499; q0x=0.003500; q0y=-0.000055
q1z=1999.207058; q1x=0.002642; q1y=-0.000164
...
######################
# dfx0 = -5*mm_to_m #(q0shft_mm[0]+bpm0x)*mm_to_m ###TODO: this is incorrect since it uses the parameters from the exit of the quad0
# dfy0 = 0*mm_to_m #(q0shft_mm[1]+bpm0y)*mm_to_m ###TODO: this is incorrect since it uses the parameters from the exit of the quad0
# fthx = -0.0007
# fthy = 0.00033
# fthz = 0
...
t_span  = (0, tmax)
max_dt  = 1e-9
dx_det_cm = -0.8 if(MagnetsSettings!=729) else +0.4 # cm
dy_det_cm = +0.3 # cm
X0 = None
t_cm = None
...
detector_x_center_cm = dx_det_cm #-0.85 if(MagnetsSettings!=729) else +0.4 # cm
detector_x_center_m = detector_x_center_cm*cm_to_m

# Define detector y range
detector_y_center_cm = 5.165 + 0.1525 + 3.685 + dy_det_cm # cm
...
def get_angles_from_dx_dy(z0,dx0,dy0):
    ty0 = abs(math.atan2(dx0,abs(z0))) ### shift in x means that px has to be rotated around y
    tx0 = abs(math.atan2(dy0,abs(z0))) ### shift in y means that py has to be rotated around x
    if(dx0>0): ty0 *= -1
    if(dy0<0): tx0 *= -1
    return tx0,ty0

def rotate(v,tx,ty,tz=0):
...
def GenerateGaussianBeam(E_GeV,mass_GeV,charge,mks=False):
    ...
    ###################
    ### find the angles of the beam from the fixed shifts
    fthx,fthy = get_angles_from_dx_dy(z0,fdx0,fdy0)
    # print(f"fthx={fthx}, fthy={fthy}")
    ### then rotate the momentum according to these angles
    if(fthx!=0 or fthy!=0):
        ### for mks
        p0  = [px0,py0,pz0]
        pR  = rotate(p0,fthx,fthy)
        px0 = pR[0]
        py0 = pR[1]
        pz0 = pR[2]
        # print(f"MKS: p0={p0}-->pr={pR}")
        ### for natural
        p0 = [px,py,pz]
        pR = rotate(p0,fthx,fthy)
        px = pR[0]
        py = pR[1]
        pz = pR[2]
        # print(f"NAT: p0={p0}-->pr={pR}")
    ### then build the state, with the shifted vertices and rotated momenta
    state_mks = [x0+fdx0,y0+fdy0,z0, px0,py0,pz0, mass_kg,charge]  ### [x[m],y[m],z[m], px[kg*m/s],py[kg*m/s],pz[kg*m/s], m[kg],q[unit]]
    state_nat = [x0+fdx0,y0+fdy0,z0, px,py,pz,    mass_GeV,charge] ### [x[m],y[m],z[m], px[GeV],py[GeV],pz[GeV], m[GeV],q[unit]]
...
if __name__ == "__main__":
    ...
    histQ0_entr_zoom = ROOT.TH2D("histQ0_entr_zoom", f"Quad_{{0}} entrance plane;x_{{LAB}} [mm];y_{{LAB}} [mm];{ztitle}", 50,-5,+5, 50,-5,+5)
    histQ1_entr_zoom = ROOT.TH2D("histQ1_entr_zoom", f"Quad_{{1}} entrance plane;x_{{LAB}} [mm];y_{{LAB}} [mm];{ztitle}", 50,-5,+5, 50,-5,+5)
    histQ2_entr_zoom = ROOT.TH2D("histQ2_entr_zoom", f"Quad_{{2}} entrance plane;x_{{LAB}} [mm];y_{{LAB}} [mm];{ztitle}", 50,-5,+5, 50,-5,+5)
    histQ0_exit_zoom = ROOT.TH2D("histQ0_exit_zoom", f"Quad_{{0}} exit plane;x_{{LAB}} [mm];y_{{LAB}} [mm];{ztitle}", 50,-5,+5, 50,-5,+5)
    histQ1_exit_zoom = ROOT.TH2D("histQ1_exit_zoom", f"Quad_{{1}} exit plane;x_{{LAB}} [mm];y_{{LAB}} [mm];{ztitle}", 50,-5,+5, 50,-5,+5)
    histQ2_exit_zoom = ROOT.TH2D("histQ2_exit_zoom", f"Quad_{{2}} exit plane;x_{{LAB}} [mm];y_{{LAB}} [mm];{ztitle}", 50,-5,+5, 50,-5,+5)
    ...
    histD_entrance.GetZaxis().SetTitleOffset(1.3)
    dipoleTPL.Draw()
    flangeTPL.Draw()
    s = ROOT.TLatex()
    s.SetNDC(1)
    ...
    s.SetTextFont(132)
    s.SetTextSize(0.045)
    s.DrawLatex(0.17,0.83,f"#mu_{{x}}={histD_entrance.GetMean(1):.1f} mm, #sigma_{{x}}={histD_entrance.GetStdDev(1):.1f} mm")
    s.DrawLatex(0.17,0.78,f"#mu_{{y}}={histD_entrance.GetMean(2):.1f} mm, #sigma_{{y}}={histD_entrance.GetStdDev(2):.1f} mm")
    #
    s = ROOT.TLatex()
    s.SetNDC(1)
    s.SetTextAlign(13)
    ...
    ### plot the divergence at the production
    fig, axs = plt.subplots(1, 2, figsize=(9, 5), tight_layout=True)
    XX = []
    PX = []
    YY = []
    ...
    plt.tight_layout()
    plt.savefig(f"{pdfname}_divergence_at_production.pdf")
    if(doshw): plt.show()
    ...
    ### get ranges
    div_xmin,  div_xmax  = np.percentile(XX, [1, 99])
    div_pxmin, div_pxmax = np.percentile(PX, [1, 99])
    div_ymin,  div_ymax  = np.percentile(YY, [1, 99])
    div_pymin, div_pymax = np.percentile(PY, [1, 99])
    ### plot the divergence
    fig, axs = plt.subplots(1, 2, figsize=(9, 5), tight_layout=True)
    hdivx = axs[0].hist2d(XX, PX, bins=(200,200), range=[[div_xmin, div_xmax], [div_pxmin, div_pxmax]], rasterized=True)
    axs[0].set_xlabel('X [m]')
    axs[0].set_ylabel('PX [GeV]')
    plt.locator_params(axis='x', nbins=10)
    plt.locator_params(axis='y', nbins=10)
    ...
    hdivy = axs[1].hist2d(YY, PY, bins=(200,200), range=[[div_ymin, div_ymax], [div_pymin, div_pymax]], rasterized=True)
    axs[1].set_xlabel('Y [m]')
    axs[1].set_ylabel('PY [GeV]')
    plt.locator_params(axis='x', nbins=10)
    plt.locator_params(axis='y', nbins=10)
    ...
    plt.tight_layout()
    plt.savefig(f"{pdfname}_divergence_at_IP_from_initstate.pdf")
    if(doshw): plt.show()
